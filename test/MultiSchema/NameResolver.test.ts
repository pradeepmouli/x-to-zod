...
import { DefaultNameResolver } from '../../src/MultiSchema/NameResolver.js';
...
describe('NameResolver - Default Implementation', () => {
  describe('resolveExportName', () => {
    ...
    it('should use last path segment for nested IDs', () => {
      ...
      expect(resolver.resolveExportName('a/b/c/test-schema')).toBe('TestSchema');
    });
    ...
    it('should handle filename strategy', () => {
      ...
      expect(resolver.resolveExportName('user-profile.schema.json')).toBe('UserProfile');
      expect(resolver.resolveExportName('schemas/post.json')).toBe('Post');
    });

    it('should apply custom transform function', () => {
      const resolver = new DefaultNameResolver('custom', (id: string) => `${id.charAt(0).toUpperCase() + id.slice(1)}Type`);
    ...
    it('should sanitize invalid identifiers', () => {
      ...
      expect(resolver.resolveExportName('test-@#$-name')).toMatch(/^[a-zA-Z_$][a-zA-Z0-9_$]*$/);
    });
  ...
  describe('edge cases', () => {
    it('should handle kebab-case correctly', () => {
      const resolver = new DefaultNameResolver();
      expect(resolver.resolveExportName('user-account-settings')).toBe('UserAccountSettings');
      expect(resolver.resolveExportName('api-v1-user-schema')).toBe('ApiV1UserSchema');
    });

    it('should handle snake_case correctly', () => {
      const resolver = new DefaultNameResolver();
      expect(resolver.resolveExportName('user_account_settings')).toBe('UserAccountSettings');
      expect(resolver.resolveExportName('api_v1_user_schema')).toBe('ApiV1UserSchema');
    });

    it('should handle mixed separators', () => {
      const resolver = new DefaultNameResolver();
      expect(resolver.resolveExportName('user-account_settings')).toBe('UserAccountSettings');
      expect(resolver.resolveExportName('api.v1.user-schema')).toBe('ApiV1UserSchema');
    });
