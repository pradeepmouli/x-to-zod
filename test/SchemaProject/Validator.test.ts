...
import type { SchemaEntry, DependencyGraph } from '../../src/SchemaProject/types.js';
...
describe('Validator', () => {
  ...
  it('detects export name conflicts', () => {
    ...
    const validator = new Validator(registry, nameResolver as any, refResolver as any, depGraph);
  ...
  it('returns valid when no errors and only non-blocking warnings exist', () => {
    ...
    registry.addEntry(makeEntry('address', {
      type: 'object',
      properties: {
        street: { type: 'string' },
      },
