...
*extends `StringFormatBuilder<ZodStringFormat<"datetime">, [params?: DatetimeParams]>`*
*implements `BuilderFor<ZodStringFormat<"datetime">>`*
```ts
constructor(version: "v3" | "v4", params?: DatetimeParams): DatetimeBuilder
```
**Properties:**
- `typeKind: "datetime"`
...
- `_params: [params?: DatetimeParams]` (optional)
- `_optional: boolean`
- `_nullable: boolean`
- `_readonly: boolean`
...
**Methods:**
- `withPrecision(precision: number): this` — Set precision for datetime validation (millisecond precision).
- `withOffset(required: boolean): this` — Require timezone offset in datetime string.
