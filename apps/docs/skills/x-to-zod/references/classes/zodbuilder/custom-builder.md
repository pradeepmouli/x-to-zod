...
*extends `BaseBuilder<ZodCustom, "custom", [params?: unknown]>`*
*implements `BuilderFor<ZodCustom>`*
```ts
constructor(version: "v3" | "v4", validateFn?: string, params?: unknown): CustomBuilder
```
**Properties:**
- `typeKind: "custom"` — Parser type discriminator (e.g. `'string'`, `'object'`, `'anyOf'`).
Set by the parser that creates this builder.
- `_validateFn: string` (optional)
- `_params: [params?: unknown]` (optional)
- `_optional: boolean`
- `_nullable: boolean`
- `_readonly: boolean`
