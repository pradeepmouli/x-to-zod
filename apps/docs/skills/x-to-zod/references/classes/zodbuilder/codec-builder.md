...
*extends `BaseBuilder<ZodCodec<In, Out>, "pipe", [inSchema: BuilderFor<In>, outSchema: BuilderFor<Out>]>`*
*implements `BuilderFor<ZodCodec>`*
```ts
constructor<In, Out>(version: "v3" | "v4", inSchema: BuilderFor<In>, outSchema: BuilderFor<Out>): CodecBuilder<In, Out>
```
**Properties:**
- `typeKind: "pipe"` — Parser type discriminator (e.g. `'string'`, `'object'`, `'anyOf'`).
Set by the parser that creates this builder.
- `_params: [inSchema: BuilderFor<In>, outSchema: BuilderFor<Out>]` (optional)
- `_optional: boolean`
- `_nullable: boolean`
- `_readonly: boolean`
