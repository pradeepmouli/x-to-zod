...
*extends `BaseBuilder<ZodLiteral, "literal", [value: Serializable]>`*
```ts
constructor(version: "v3" | "v4", value: Serializable): LiteralBuilder
```
**Properties:**
- `typeKind: "literal"` — Parser type discriminator (e.g. `'string'`, `'object'`, `'anyOf'`).
