...
/**
 * Build a Zod string schema string from JSON schema string constraints.
 * Preserves exact modifier order and error message formatting.
 *
 * Note: This function requires parseSchema for contentSchema delegation,
 * so it accepts it as a parameter to avoid circular dependencies.
 */
export function buildString(
  schema: JsonSchemaObject & { type: "string" },
  parseSchema: (schema: any, refs?: any) => string,
): string {
  let r = "z.string()";
