...
export class DefaultRefResolver {
	...
	private computeModulePath(
		...
		const targetDir = targetSchemaId.substring(
			0,
			targetSchemaId.lastIndexOf('/'),
		);

		// If in same directory, use relative import
		if (fromDir === targetDir) {
			const targetName = targetSchemaId.substring(
				targetSchemaId.lastIndexOf('/') + 1,
			);
			return `./${targetName}`;
		}
